...
toJSONDeclarationMapJson (DeclarationMapJson a b) =
  A.toJSON (toJSONFgPackage a, toJSONModuleDeclarations b)
...
parseJSONDeclarationMapJson = A.withArray "DeclarationMapJson" $ \vec -> do
  !mVal0 <- V.indexM vec 0
  !mVal1 <- V.indexM vec 1
  !val0 <- parseJSONFgPackage mVal0
  !val1 <- parseJSONModuleDeclarations mVal1
